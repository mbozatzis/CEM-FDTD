...
grid_center = [floor(size(e, 1)/2), floor(size(e, 2)/2), floor(size(e, 3)/2)];

for i = 1:N_x_new
    for j = 1:N_y_new
        for mk = 1:N_z_new
            if (i >= grid_center(1) - Lx/2) && (i <= grid_center(1) + Lx/2) ...
                    && (j >= grid_center(2) - Ly/2) && (j <= grid_center(2) + Ly/2)
                if (mk > grid_center(3) - h/2) && (mk <= grid_center(3) + h/2)
                    e(i, j, mk) = 2.2 * e0;
                elseif mk == floor(grid_center(3) - h/2)
                    e(i, j, mk) = Inf;
                end
            end
        end
...
Ai = linspace(grid_center(1) - Lx/2 + 3, grid_center(1) + Lx/2 - 3, 8);
Aj = linspace(grid_center(2) - Ly/2 + 8, grid_center(2) + Ly/2 - 8, 6);

for i = 1:length(Ai)
    j0 = grid_center(2) - Ly/2 + 3;
    j1 = grid_center(2) + Ly/2 - 3;
    e = createCircle(e, 2.2, round(Ai(i)), j0, floor(grid_center(3) - h/2), sr);
    e = createCircle(e, 2.2, round(Ai(i)), j1, floor(grid_center(3) - h/2), sr);
end

for j = 1:length(Aj)
    i0 = grid_center(1) - Lx/2 + 3;
    i1 = grid_center(1) + Lx/2 - 3;
    e = createCircle(e, 2.2, i0, round(Aj(j)), floor(grid_center(3) - h/2), sr);
    e = createCircle(e, 2.2, i1, round(Aj(j)), floor(grid_center(3) - h/2), sr);
end
...
for t = 0:dt:Tmax
    ...
    A = Ez(Npml+1:Npml+Ny, 41, Npml+1:Npml+Ny);
    A = reshape(A(:, 1, :), [size(A, 1), size(A,3)]);
    pcolor(A);
    shading interp;
    drawnow;
end
