...
ind_t2 = ceil((7/12)*i_max); % t2 = 7T0
ind_t3 = ceil((10/12)*i_max); % t3 = 10T0
ind_t4 = i_max; % t4 = 12T0 = TnT0
...
t = linspace(0, 12, length(Ez_nb));

figure(5);
sgtitle('Electric Field at points of grid over time');

subplot(4, 2, 1);
plot(t, reshape(Ez_nb(x1, y1, :), [1, length(Ez_nb)]));
title('Point x1 with no boundary conditions');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 2);
plot(t, reshape(Ez_nb(x2, y2, :), [1, length(Ez_nb)]));
title('Point x2 with no boundary conditions');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 3);
plot(t, reshape(Ez_m1(x1, y1, :), [1, length(Ez_m1)]));
title('Point x1 with first order Mur');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 4);
plot(t, reshape(Ez_m1(x2, y2, :), [1, length(Ez_m1)]));
title('Point x2 with first order Mur');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 5);
plot(t, reshape(Ez_m2(x1, y1, :), [1, length(Ez_m2)]));
title('Point x1 with second order Mur');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 6);
plot(t, reshape(Ez_m2(x2, y2, :), [1, length(Ez_m2)]));
title('Point x2 with second order Mur');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 7);
plot(t, reshape(Ez_p(x1, y1, :), [1, length(Ez_p)]));
title('Point x1 PML');
ylabel('Ez field');
xlabel('Time (T_0)');

subplot(4, 2, 8);
plot(t, reshape(Ez_p(x2, y2, :), [1, length(Ez_p)]));
