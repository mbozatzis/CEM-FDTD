...
for t = 0:dt:Tmax
    ...
    for i = 1:Npml-1
        ...
        for j = 1:N_y-1
            ...
            if j == 1
                Ezy_pml_h(i, j, 2) = Ca_pml(k)*Ezy_pml_h(i, j, 2) + Cb_pml(k)*(Hx_pml_c23(i, Npml) ...
                    - Hx_pml_h(i, j, 2));
            else
    ...
    for i = 1:Npml-1
        for j = 1:Npml-1
            k = Npml-j+1;
            if i == 1
                Ezx_pml_c34(i, j) = Ca_pml(k)*Ezx_pml_c34(i, j) + Cb_pml(k)*(Hy_pml_c34(i, j) ...
                    - Hy_pml_v(N_x, j, 2));
            else
                Ezx_pml_c34(i, j) = Ca_pml(k)*Ezx_pml_c34(i, j) + Cb_pml(k)*(Hy_pml_c34(i, j) ...
                    - Hy_pml_c34(i-1, j));
            ...
            if j == 1
                Ezy_pml_c34(i, j) = Ca_pml(k)*Ezy_pml_c34(i, j) + Cb_pml(k)*(Hx_pml_h(i, N_y, 2) ...
                    - Hx_pml_c34(i, j));
            else
                Ezy_pml_c34(i, j) = Ca_pml(k)*Ezy_pml_c34(i, j) + Cb_pml(k)*(Hx_pml_c34(i, j-1) ...
                    - Hx_pml_c34(i, j));
            end
        end
    end
    ...
    for i = 1:Npml-1
        for j = 2:Npml
            k = Npml-j+1;
            if i == 1
                Ezx_pml_c23(i, j) = Ca_pml(k)*Ezx_pml_c23(i, j) + Cb_pml(k)*(Hy_pml_c23(i, j) ...
                    - Hy_pml_v(N_x, j, 1));
            else
                Ezx_pml_c23(i, j) = Ca_pml(k)*Ezx_pml_c23(i, j) + Cb_pml(k)*(Hy_pml_c23(i, j) ...
                    - Hy_pml_c23(i-1, j));
            end
            Ezy_pml_c23(i, j) = Ca_pml(k)*Ezy_pml_c23(i, j) + Cb_pml(k)*(Hx_pml_c23(i, j-1) ...
                - Hx_pml_c23(i, j));
        end
    end
        ...
        for j = 1:N_y-1
            if j == N_y
                Hx_pml_h(i, j, 2) = Dax_pml(k)*Hx_pml_h(i, j, 2) + Dbx_pml(k)*(Ezx_pml_h(i, j, 2) ...
                    + Ezy_pml_h(i, j, 2) - Ezx_pml_c34(i, 1) - Ezy_pml_c34(i, 1));
            else
                Hx_pml_h(i, j, 2) = Dax_pml(k)*Hx_pml_h(i, j, 2) + Dbx_pml(k)*(Ezx_pml_h(i, j, 2) ...
    ...
    for i = 2:N_x
        for j = 2:Npml
            if i == N_x
                Hy_pml_v(i, j, 1) = Day_pml(j)*Hy_pml_v(i, j, 1) + Dby_pml(j)*(Ezx_pml_c23(1, j) ...
                    + Ezy_pml_c23(1, j) - Ezx_pml_v(i, j, 1) - Ezy_pml_v(i, j, 1));
            else
    ...
    for i = 2:N_x
        for j = 1:Npml-1
            ...
            if i == N_x
                Hy_pml_v(i, j, 2) = Day_pml(k)*Hy_pml_v(i, j, 2) + Dby_pml(k)*(Ezx_pml_c34(1, j) ...
                    + Ezy_pml_c34(1, j) - Ezx_pml_v(i, j, 2) - Ezy_pml_v(i, j, 2));
            else
    ...
    for i = 1:Npml-1
        for j = 1:Npml-1
            k = Npml-j+1;
            Hx_pml_c34(i, j) = Dax_pml(k)*Hx_pml_c34(i, j) + Dbx_pml(k)*(Ezx_pml_c34(i, j) ...
                + Ezy_pml_c34(i, j) - Ezx_pml_c34(i, j+1) - Ezy_pml_c34(i, j+1));
            Hy_pml_c34(i, j) = Day_pml(k)*Hy_pml_c34(i, j) + Dby_pml(k)*(Ezx_pml_c34(i+1, j) ...
                + Ezy_pml_c34(i+1, j) - Ezx_pml_c34(i, j) - Ezy_pml_c34(i, j));
        end
    end
    ...
    for i = 1:Npml-1
        for j = 2:Npml
            k = Npml-j+1;
            if j == Npml
                Hx_pml_c23(i, j) = Dax_pml(k)*Hx_pml_c23(i, j) + Dbx_pml(k)*(Ezx_pml_c23(i, j) ...
                    + Ezy_pml_c23(i, j) - Ezx_pml_h(i, 1, 2) - Ezy_pml_h(i, 1, 2));
            else
                Hx_pml_c23(i, j) = Dax_pml(k)*Hx_pml_c23(i, j) + Dbx_pml(k)*(Ezx_pml_c23(i, j) ...
                    + Ezy_pml_c23(i, j) - Ezx_pml_c23(i, j+1) - Ezy_pml_c23(i, j+1));
            end
            Hy_pml_c23(i, j) = Day_pml(k)*Hy_pml_c23(i, j) + Dby_pml(k)*(Ezx_pml_c23(i+1, j) ...
                + Ezy_pml_c23(i+1, j) - Ezx_pml_c23(i, j) - Ezy_pml_c23(i, j));
        end
    end
